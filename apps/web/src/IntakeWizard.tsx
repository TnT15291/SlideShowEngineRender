...
function RecipeFit({ intro }: { intro: { photos: string; story: string; music: string } }) {
  return <div className="mt-4 grid gap-4 rounded-lg border bg-secondary/40 p-4 text-sm sm:grid-cols-3">
    <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Ảnh phù hợp</p><p className="mt-1 leading-5">{intro.photos}</p></div>
    <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Câu chuyện phù hợp</p><p className="mt-1 leading-5">{intro.story}</p></div>
    <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Nhạc phù hợp</p><p className="mt-1 leading-5">{intro.music}</p></div>
  </div>
}
