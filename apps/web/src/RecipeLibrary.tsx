  ...
  return (
    <>
      <header className="flex h-20 items-center gap-4 border-b px-6 md:px-10">
        <button aria-label="Open navigation" onClick={onOpenNav} className="grid size-11 shrink-0 place-items-center rounded-md border lg:hidden"><Menu className="size-4" /></button>
      ...
      <div className="mx-auto max-w-[1440px] px-6 py-8 md:px-10">
        <div className="mb-6 grid gap-3 md:grid-cols-[minmax(260px,1fr)_220px_180px]">
          ...
          <select className="field mt-0" value={mood} onChange={(event) => setMood(event.target.value)} aria-label="Filter by mood"><option value="">All moods</option>{moods.map((item) => <option key={item} value={item}>{humanizeTag(item)}</option>)}</select>
          <input className="field mt-0" type="number" min="1" value={photoCount} onChange={(event) => setPhotoCount(event.target.value)} aria-label="Show only styles that suit this many photos" placeholder="How many photos?" />
        </div>
...
function RecipeCard({ recipe, onOpen }: { recipe: RecipeSummary; onOpen: () => void }) {
  ...
  return <button onClick={onOpen} className="text-left"><Card className="h-full overflow-hidden transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md">
    ...
    <CardContent className="space-y-4 pt-0">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-muted-foreground">{range && <span className="flex items-center gap-1.5"><Image className="size-3.5" /> {range}</span>}<span className="flex items-center gap-1.5"><Layers3 className="size-3.5" /> {recipe.sceneCount} scenes · {recipe.signatureCount} looks</span>{recipe.energy && <span className="flex items-center gap-1.5"><Music2 className="size-3.5" /> {humanizeTag(recipe.energy)} energy</span>}</div>
...
function RecipeDetail({ recipe, onClose }: { recipe: RecipeSummary; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/35" onClick={onClose}><aside className="h-full w-full max-w-xl overflow-y-auto bg-background p-7 shadow-2xl" onClick={(event) => event.stopPropagation()}>
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-primary">Recipe details</p><h2 className="mt-2 font-serif text-3xl font-semibold">{recipe.name}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{recipe.intro?.summary || recipe.notes}</p></div><Button variant="ghost" size="icon" onClick={onClose}><X className="size-4" /></Button></div>
    <div className="mt-8 grid grid-cols-3 gap-3"><DetailStat label="Scenes" value={String(recipe.sceneCount)} /><DetailStat label="Looks" value={`${recipe.signatureCount} on ${recipe.layoutCount}`} /><DetailStat label="Energy" value={recipe.energy || "Flexible"} /></div>
    {recipe.intro && <section className="mt-8"><h3 className="text-sm font-semibold">Best fit</h3><div className="mt-3 space-y-3 rounded-lg border bg-secondary/40 p-4 text-sm"><p><span className="font-medium">Photos: </span><span className="text-muted-foreground">{recipe.intro.photos}</span></p><p><span className="font-medium">Story: </span><span className="text-muted-foreground">{recipe.intro.story}</span></p><p><span className="font-medium">Music: </span><span className="text-muted-foreground">{recipe.intro.music}</span></p></div></section>}
    ...
    <section className="mt-8"><h3 className="text-sm font-semibold">Story arc</h3><div className="mt-3 flex flex-wrap gap-2">{recipe.storyArc.map((beat, index) => <span key={`${beat}-${index}`} className="rounded-full bg-secondary px-3 py-1.5 text-xs">{index + 1}. {humanizeTag(beat)}</span>)}</div></section>
    ...
    <section className="mt-8"><h3 className="text-sm font-semibold">Best for</h3><div className="mt-3 flex flex-wrap gap-2">{recipe.bestFor.map((tag) => <Badge key={tag} variant="outline">{humanizeTag(tag)}</Badge>)}</div></section>
  </aside></div>
}
