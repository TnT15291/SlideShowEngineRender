...
function RecipeDetail({ recipe, onClose }: { recipe: RecipeSummary; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/35" onClick={onClose}><aside className="h-full w-full max-w-xl overflow-y-auto bg-background p-7 shadow-2xl" onClick={(event) => event.stopPropagation()}>
    ...
    {recipe.intro && <section className="mt-8"><h3 className="text-sm font-semibold">Phù hợp với</h3><div className="mt-3 space-y-3 rounded-lg border bg-secondary/40 p-4 text-sm"><p><span className="font-medium">Ảnh: </span><span className="text-muted-foreground">{recipe.intro.photos}</span></p><p><span className="font-medium">Câu chuyện: </span><span className="text-muted-foreground">{recipe.intro.story}</span></p><p><span className="font-medium">Nhạc: </span><span className="text-muted-foreground">{recipe.intro.music}</span></p></div></section>}
